...
export default function RankingImgs() {
  ...
  const getRank = async (gender: string) => {
    return await axiosCustom
      .get(`/api/v1/styles?gender=${gender}`)
      .then((res) => res.data);
  };

  const { data: maleRank } = useQuery(
    ['male'],
    async () => await getRank('male'),
    {
  ...
  const { data: femaleRank } = useQuery(
    ['female'],
    async () => await getRank('female'),
    {
      staleTime: 1000 * 60 ** 60,
      refetchOnWindowFocus: false,
    },
  ...
  console.log(maleRank, femaleRank);
  ...
  const SVGComponents: SVGComponentsType = {
    female: {
      ...
      top: {
        blouse: (
          <SVGFemaleBlouse
            main={colorArr[femaleRank?.top_color]}
            sub={sub}
            detail={detail}
          ></SVGFemaleBlouse>
        ),
      ...
      bottom: {
        denim: (
          <SVGFemaleDenim
            main={colorArr[femaleRank?.bottom_color]}
            sub={sub}
            detail={detail}
          ></SVGFemaleDenim>
        ),
    ...
    male: {
      ...
      top: {
        blouse: (
          <SVGMaleBlouse
            main={colorArr[maleRank?.top_color]}
            sub={sub}
            detail={detail}
          ></SVGMaleBlouse>
        ),
      ...
      bottom: {
        denim: (
          <SVGMaleDenim
            main={colorArr[maleRank?.bottom_color]}
            sub={sub}
            detail={detail}
          ></SVGMaleDenim>
        ),
          ...
          <Box sx={{ display: 'flex' }}>
            ...
            >
              ...
              >
                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['background']['background']['background1']}
                </Box>
                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['male']['hair']['middle']}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['male']['inner']['basic_t_shirts']}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['male']['bottom'][maleRank?.bottom]}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['male']['top'][maleRank?.top]}
                </Box>
              </Box1>
              <Typography sx={{ marginTop: 4 }} variant="h4">
                {maleRank?.count}'s pick
              </Typography>
            </Box>
            <Box
              sx={{
            ...
            >
              ...
              >
                ...
                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['female']['hair']['long']}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['female']['inner']['basic_t_shirts']}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['female']['bottom'][femaleRank?.bottom]}
                </Box>

                <Box sx={{ position: 'absolute' }} width="100%" height="100%">
                  {SVGComponents['female']['top'][femaleRank?.top]}
                </Box>
              </Box1>
              <Typography sx={{ marginTop: 4 }} variant="h4">
                {maleRank?.count}'s pick
